...
| Token | The unit models use to measure text (roughly 3/4 of a word). Usage limits and API billing are counted in tokens. You will mostly encounter this via `/cost` or the context indicator. |
...
| Accept Edits mode | A mode that auto-approves file edits for the rest of the session, while other actions such as shell commands still prompt for approval. Useful once you trust the direction of the work. Toggle with Shift+Tab. |
...
| Subagent | A secondary Claude instance that the main session can spawn to handle a focused subtask, such as running tests, searching a large codebase, or reviewing code, in its own context window before reporting back. Configured via `/agents`. |
...
| Skill | A packaged set of instructions and helper files that teaches Claude a specific workflow. Skills are invoked with `/` like built-in commands and can also load automatically when relevant. |
| Command | Any input starting with `/`. Built-in commands control the session; custom ones are skills your team defines in `.claude/skills/<name>/SKILL.md`. |
